...
for i in range(500):
    tempSong = song[-100:]
    #model.reset_states()
    outputHot = model.predict(np.array(tempSong).reshape(1,100,20))
    #print(outputHot)
    max_num = 0
    count = 0
    max_count = 0
    for item in outputHot[0]:
        if item > max_num:
            max_num=item
            max_count = count
        count+=1
    new_hot = [0]*len(outputHot[0])
    new_hot[max_count] = 1
    print(max_count)
    song.append(w2v[reverseEncoderDict[str(new_hot)]])
p.save( song[99:], 'song' )
